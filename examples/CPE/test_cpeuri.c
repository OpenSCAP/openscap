...
// string representation of entries in enum CpePart_t
const char* CPE_PART_STR[] = { "??? ERROR - run away!", "hardware", "operating system", "application" };
...
int main(int argc, char** argv)
{
	...
	if (argc < 2) {
		...
		if (argc != 3) {
			// not enough arguments
			fprintf(stderr, "ERROR: URI to parse was not specified!\n");
			return EXIT_FAILURE;
		}
		...
		if (argc < 4) {
			// not enough arguments
			fprintf(stderr, "ERROR: At least two CPE URIs are needed to perform CPE names matching!\n");
			return EXIT_FAILURE;
		}
		...
		// interpret result
		if (result == -2)
			fprintf(stderr, "One of given strings is not a valid CPE URI.\n");
		else if (result == -1)

...
void test_platformspec_dump(CpePlatformSpec_t* plat)
{
	int i;
...
void test_langexpr_dump(CpeLangExpr_t* expr, int depth)
{
	...
	printf("+");
	// print operand
	if (expr->oper & CPE_LANG_OPER_NOT) printf("N");
...
{
	CpePlatformSpec_t* plat; // pointer to our platform specification
	int i;
	...
	if (plat != NULL) {
		...
		for (i = 0; i < plat->platforms_n; ++i) {
			...
			printf("    = %sMATCH\n\n", cpe_language_match_cpe(cpes, cpes_n, plat->platforms[i]) ? "" : "MIS");
		}
	}

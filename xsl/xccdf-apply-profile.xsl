...
<xsl:template mode='profile' match='cdf:Group|cdf:Rule'>
  ...
  <xsl:variable name='sel' select='$p/cdf:select[@idref=$id]'/>
  <xsl:variable name='rr' select='$p/cdf:refine-rule[@idref=$id]'/>
  <xsl:copy>
    <xsl:copy-of select='@*'/>
...
<xsl:template mode='profile' match='cdf:check'>
  ...
  <xsl:variable name='rr' select='$p/cdf:refine-rule[@idref=$rule/@id]'/>
...
<xsl:template mode='profile' match='cdf:Value'>
  <xsl:param name='p'/>
  <xsl:variable name='id' select='@id'/>
  <xsl:variable name='rv' select='$p/cdf:refine-value[@idref=$id]'/>
  <xsl:variable name='sv' select='$p/cdf:set-value[@idref=$id]'/>
  <xsl:copy>
    <xsl:copy-of select='@*'/>

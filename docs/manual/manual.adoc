...
is shown for XCCDF files and is sourced from the XCCDF **status** element.
To get this date when using Source DataStreams you have to split the datastream
first using `oscap ds sds-split` and then use `oscap info` on the XCCDF.
...
You can get both dates in a more human readable format if you generate an HTML
report from the XCCDF result or ARF. Use the `oscap xccdf generate report`
command.

...
If you want to specify whether your OVAL result should be "full" or "thin",
you can set it using OVAL directives file. This file can be loaded be OpenSCAP
by *--directives <file>* option.
...
Another place, where these identifiers are located is machine-readable Result
Datastream files.
...
correctly, oscap tool needs to fetch remote up-to-date OVAL file. This can be
...
Despite of external tailoring file, you can also use tailoring component integrated to data stream.
...
We will choose "scap_org.open-scap_cref_simple-tailoring.xml" tailoring component.
...
Specified XCCDF or data stream content can contains zero or more profiles.

You can select existing profile using *--profile* option, or you can evaluate scan without

...
int main(int argc, char **argv)
{
	...
	while (ds_stream_index_iterator_has_more(streams))
	{
		...
		if (strcmp(ds_stream_index_get_id(stream), "scap_org.open-scap_datastream_from_xccdf_scap-fedora14-xccdf.xml") != 0)
		{
			printf("Failed to read datastream ID correctly. Expected 'scap_org.open-scap_datastream_from_xccdf_scap-fedora14-xccdf.xml', found '%s'.\n", ds_stream_index_get_id(stream));
			return 1;
		}
	}

...
function test_probes_filehash58 {
    ...
    if [ ! -x ${OVAL_PROBE_DIR}/probe_filehash58 ]; then		
	echo -e "Probe filehash58 does not exist!\n" 
	return 255; # Test is not applicable.
    fi

    local ret_val=0;
    local DEFFILE="test_probes_filehash58.xml"
    ...
    bash ${srcdir}/test_probes_filehash58.xml.sh > $DEFFILE

...
struct xccdf_item* xccdf_item_new(xccdf_type_t type, struct xccdf_item* bench, struct xccdf_item* parent)
{
    ...
    switch (type) {
        case XCCDF_BENCHMARK:  size += sizeof(struct xccdf_benchmark_item); break;
        case XCCDF_RULE:       size += sizeof(struct xccdf_rule_item);      break;
        case XCCDF_GROUP:      size += sizeof(struct xccdf_group_item);     break;
        case XCCDF_VALUE:      size += sizeof(struct xccdf_value_item);     break;
        case XCCDF_RESULT:     size += sizeof(struct xccdf_result_item);    break;
	...
	else item->item.benchmark = bench;
...
void xccdf_item_dump(struct xccdf_item* item, int depth)
{
    if (item == NULL) return;
    switch (item->type) {
        case XCCDF_BENCHMARK: xccdf_benchmark_dump(XBENCHMARK(item)); break;
        case XCCDF_GROUP:     xccdf_group_dump(item, depth);          break;
        case XCCDF_RULE:      xccdf_rule_dump(item, depth);           break;
        default: xccdf_print_depth(depth); fprintf(stderr, "I cannot yet dump an item of type no. %u.", item->type);
    }
}
...
bool xccdf_item_process_attributes(struct xccdf_item* item, xmlTextReaderPtr reader)
{
	...
	if (xccdf_attribute_has(reader, XCCDFA_EXTENDS))
		xccdf_benchmark_add_ref(item->item.benchmark, &item->item.extends, xccdf_attribute_get(reader, XCCDFA_EXTENDS), item->type);
...
XCCDF_ITEM_GETTER(time_t, version_time)

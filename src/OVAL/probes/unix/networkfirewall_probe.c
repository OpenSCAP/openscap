...
const char* get_iifname(struct rule* rule) {
    ...
    json_array_foreach(expr, index, expr_child) {
        if (json_unpack(expr_child, "{s:{s:{s:s}, s:s,s?s}}", "match", "left", "meta", &meta_value, "right", &interface_name,"op",&op_value) == 0) {
...
const char *get_protocol(struct rule* rule) {
    ...
    json_array_foreach(expr, index, expr_child) {
        ...
        if (json_unpack(expr_child, "{s:{s:{s:{s:s, s:s}}, s:s, s?:s}}", "match", "left", "payload", "name", &ip_field_name, "field", &field_name, "right", &str_field_value, "op", &op_value) == 0) {
            if (op_value != NULL) {
                continue;
            }
        ...
        if (json_unpack(expr_child, "{s:{s:{s:{s:s}}, s:s, s?:s}}","match", "left", "ct", "key", &key_value, "right", &protocol_name, "op", &op_value) == 0) {
            if (op_value != NULL) {
                continue;
            }
        ...
        // [tcp, udp, sctp] xxx
        if (json_unpack(expr_child, "{s:{s:{s:{s:s, s:s}}, s:o, s?:s}}", "match", "left", "payload", "name", &protocol_name, "field", &field_name, "right",  &field_value, "op", &op_value) == 0) {
            if (op_value != NULL) {
                continue;
            }
            if (strncmp(protocol_name, "tcp", 3) == 0
...
struct addr_info get_saddr(struct rule* rule) {
    ...
    json_array_foreach(expr, index, expr_child) {
        // [ip,ip6] saddr xxx
        if(json_unpack(expr_child, "{s:{s:{s:{s:s, s:s}}, s:s, s?:s}}", "match", "left", "payload", "name", &payload_name, "field", &payload_field ,"right", &field_value, "op", &op_value) == 0) {
            if (op_value != NULL) {
                continue;
            }
        ...
        // [ip,ip6] saddr xxx/yy
        if (json_unpack(expr_child, "{s:{s:{s:{s:s, s:s}}, s:{s:{s:s, s:i}}, s?:s}}", "match", "left", "payload", "name", &payload_name, "field", &payload_field, "right", "prefix", "addr", &field_value, "len", &prefix_len, "op", &op_value) == 0) {
            if (op_value != NULL) {
                continue;
            }
            if (strncmp(payload_name, "ip", 2) == 0
        ...
        if (json_unpack(expr_child, "{s:{s:{s:{s:s, s:s, s:s}}, s:s, s?:s}}", "match",
                "left", "ct", "key", &key, "dir", &dir, "family",
                &family, "right", &key_value, "op", &op_value) == 0) {
            if (op_value != NULL) {
                    continue;
            }
        ...
        if (json_unpack(expr_child,
                ...
                &key_value, "len", &prefix_len, "op", &op_value) == 0) {
            if (op_value != NULL) {
                    continue;
            }
            if (strncmp(family, "ip", 2) == 0 &&
...
int get_sport(struct rule* rule) {
    ...
    json_array_foreach(expr, index, expr_child) {
        if (json_unpack(expr_child, "{s:{s:{s:{s:s, s:s}}, s:i}}", "match", "left", "payload", "name", &ip_field_name, "field", &field_name, "right", &int_field_value) == 0) {
...
int networkfirewall_probe_main(probe_ctx *ctx, void *arg) {
    ...
    SEXP_t *output_interface = probe_obj_getent(probe_in, "output_interface", 1);

/**
 * @file   auditdline_probe.c
 * @brief  auditdline probe
			...
			if (r->mask[word] & bit) {
				...
				if (!count) {
					syscall_buf_len =
						snprintf(NULL, 0, " -S ");
					char *syscall_buf =
						malloc(syscall_buf_len + 1);
					snprintf(syscall_buf,
						 syscall_buf_len + 1, " -S ");
					audit_line_push(audit_line,
							syscall_buf);
					free(syscall_buf);
				}
				if (ptr) {
...
static int print_arch(unsigned int value, int op, struct audit_line *audit_line)
{
	...
	if (machine < 0) {
		arch_buf_len =
			snprintf(NULL, 0, " -F arch%s0x%X",
				 audit_operator_to_symbol(op), (unsigned)value);
		char *arch_buf = malloc(arch_buf_len + 1);
		snprintf(arch_buf, arch_buf_len + 1, " -F arch%s0x%X",
			 audit_operator_to_symbol(op), (unsigned)value);
		...
		if (interpret == 0) {
			if (__AUDIT_ARCH_64BIT & _audit_elf) {
				arch_buf_len =
					snprintf(NULL, 0, " -F arch%sb64",
						 audit_operator_to_symbol(op));
				char *arch_buf = malloc(arch_buf_len + 1);
				snprintf(arch_buf, arch_buf_len + 1,
					 " -F arch%sb64",
					 audit_operator_to_symbol(op));
				audit_line_push(audit_line, arch_buf);
				free(arch_buf);
			...
			arch_buf_len =
				snprintf(NULL, 0, " -F arch%s%s",
					 audit_operator_to_symbol(op), ptr);
			char *arch_buf = malloc(arch_buf_len + 1);
			snprintf(arch_buf, arch_buf_len + 1, " -F arch%s%s",
				 audit_operator_to_symbol(op), ptr);
			audit_line_push(audit_line, arch_buf);
			free(arch_buf);
		}
		...
		if (name) {
			...
			if (field == AUDIT_MSGTYPE) {
				if (!audit_msg_type_to_name(r->values[i])) {
					field_value_len = snprintf(
						NULL, 0, " -F %s%s%d", name,
						audit_operator_to_symbol(op),
						r->values[i]);
					char *audit_msg_buf =
						malloc(field_value_len + 1);
					snprintf(audit_msg_buf,
						 field_value_len + 1,
						 " -F %s%s%d", name,
						 audit_operator_to_symbol(op),
						 r->values[i]);
				   ...
				   field != AUDIT_PPID) {
				field_value_len =
					snprintf(NULL, 0, " -F %s%s%.*s", name,
						 audit_operator_to_symbol(op),
						 r->values[i],
						 &r->buf[boffset]);
				char *audit_field = malloc(field_value_len + 1);
				snprintf(audit_field, field_value_len + 1,
					 " -F %s%s%.*s", name,
					 audit_operator_to_symbol(op),
				...
				if (watch) {
					...
					char *audit_field =
						malloc(field_value_len + 1);
					snprintf(audit_field,
						 field_value_len + 1, "-w %.*s",
						 r->values[i],
						 &r->buf[boffset]);
					audit_line_push(audit_line,
							audit_field);
			...
			field_value_len =
				snprintf(NULL, 0, " f%d%s%d", r->fields[i],
					 audit_operator_to_symbol(op),
					 r->values[i]);
			char *audit_field = malloc(field_value_len + 1);
			snprintf(audit_field, field_value_len + 1, " f%d%s%d",
				 r->fields[i], audit_operator_to_symbol(op),
				 r->values[i]);
			audit_line_push(audit_line, audit_field);
			free(audit_field);
		}
	...
	for (i = 0; i < timeout; i++) {
		...
		if (retval > 0) {
			if (rep.type == NLMSG_ERROR && rep.error->error == 0) {
				...
				dE("Error while reading audit rules packets from the kernel : %d",
				   errnum);
				SEXP_t *item =
					probe_item_create(OVAL_LINUX_AUDITDLINE,
							  NULL, "filter_key",
							  OVAL_DATATYPE_STRING,
							  "", NULL);

				probe_item_setstatus(item,
						     SYSCHAR_STATUS_ERROR);
				probe_item_add_msg(
					item, OVAL_MESSAGE_LEVEL_ERROR,
					"Error while reading audit rules packets from the kernel : %d",
					errnum);
...
static struct oscap_list *get_all_audit_rules(probe_ctx *ctx)
{
	...
	if (audit_fd > -1) {
		...
		if (audit_request_rules_list_data(audit_fd) > 0) {
			...
			struct oscap_list *audit_list =
				get_reply(audit_fd, ctx);
			audit_close(audit_fd);
			return audit_list;
		} else {
			int errnum = errno;
			dE("Failed to request the kernel for audit rules : %s",
			   strerror(errnum));
			SEXP_t *item = probe_item_create(OVAL_LINUX_AUDITDLINE,
							 NULL, "filter_key",
							 OVAL_DATATYPE_STRING,
							 "", NULL);
			...
			probe_item_add_msg(
				item, OVAL_MESSAGE_LEVEL_ERROR,
				"Failed to request the kernel for audit rules : %s.",
				strerror(errnum));
			probe_item_collect(ctx, item);
		}
        audit_close(audit_fd);
		...
		dE("Failed to get a handle for the audit system : %s",
		   strerror(errnum));
		SEXP_t *item =
			probe_item_create(OVAL_LINUX_AUDITDLINE, NULL,
		...
		probe_item_add_msg(
			item, OVAL_MESSAGE_LEVEL_ERROR,
			"Failed to obtain a handle to the audit system : %s.",
			strerror(errnum));
		probe_item_collect(ctx, item);
	}
...
int auditdline_probe_main(probe_ctx *ctx, void *arg)
{
	...
	if (audit_list != NULL) {
		struct oscap_iterator *audit_list_iter =
			oscap_iterator_new(audit_list);
		while (oscap_iterator_has_more(audit_list_iter)) {
			struct audit_line *audit_line =
				oscap_iterator_next(audit_list_iter);
			if (!isNil) {
				struct oscap_string_iterator *filter_key_iter =
					oscap_stringlist_get_strings(
						audit_line->filter_key);
				while (oscap_string_iterator_has_more(
					filter_key_iter)) {
					const char *filter_key =
						oscap_string_iterator_next(
							filter_key_iter);
					SEXP_t *sexp_filter_key =
						SEXP_string_newf("%s",
								 filter_key);
					if (probe_entobj_cmp(key_filter,
							     sexp_filter_key) ==
					    OVAL_RESULT_TRUE) {
						SEXP_t *item = probe_item_create(
							OVAL_LINUX_AUDITDLINE,
							NULL, "filter_key",
							OVAL_DATATYPE_STRING,
							filter_key, "auditline",
							OVAL_DATATYPE_STRING,
				...
				SEXP_t *item = probe_item_create(
					...
					"auditline", OVAL_DATATYPE_STRING,
					oscap_string_get_cstr(
						audit_line->audit_line),
					"line_number", OVAL_DATATYPE_INTEGER,
					audit_line->line_number, NULL);
				probe_item_setstatus(item,
						     SYSCHAR_STATUS_EXISTS);
				probe_item_collect(ctx, item);
			}

...
static int _oval_sysinfo_parse_tag(xmlTextReaderPtr reader, struct oval_parser_context *context, void *user)
{
	...
	if (is_ovalsys && (strcmp(tagname, "os_name") == 0)) {
		return_code = oval_parser_text_value(reader, &_oval_sysinfo_parse_tag_consume_os_name, sysinfo);
	} else if (is_ovalsys && (strcmp(tagname, "os_version") == 0)) {
		return_code = oval_parser_text_value(reader, &_oval_sysinfo_parse_tag_consume_os_version, sysinfo);
	} else if (is_ovalsys && (strcmp(tagname, "architecture") == 0)) {
		return_code = oval_parser_text_value(reader, &_oval_sysinfo_parse_tag_consume_os_architecture, sysinfo);
	} else if (is_ovalsys && (strcmp(tagname, "primary_host_name") == 0)) {
		return_code = oval_parser_text_value(reader,  &_oval_sysinfo_parse_tag_consume_primary_host_name, sysinfo);

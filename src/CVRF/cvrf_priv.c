...
struct cvrf_involvement *cvrf_involvement_clone(const struct cvrf_involvement *involve) {
	struct cvrf_involvement *clone = oscap_calloc(1, sizeof(struct cvrf_involvement));
...
struct cvrf_acknowledgment *cvrf_acknowledgment_clone(const struct cvrf_acknowledgment *ack) {
	struct cvrf_acknowledgment *clone = oscap_calloc(1, sizeof(struct cvrf_acknowledgment));
...
struct cvrf_doc_publisher *cvrf_doc_publisher_parse(xmlTextReaderPtr reader) {
	...
	publisher->publisher_type = cvrf_item_parse_type_attribute(reader, CVRF_DOCUMENT_PUBLISHER);
...
struct cvrf_reference *cvrf_reference_parse(xmlTextReaderPtr reader) {
	...
	if (xmlTextReaderGetAttribute(reader, BAD_CAST "Type") != NULL)
		ref->ref_type = cvrf_item_parse_type_attribute(reader, CVRF_REFERENCE);
...
struct cvrf_branch *cvrf_branch_parse(xmlTextReaderPtr reader) {
	...
	branch->branch_type = cvrf_item_parse_type_attribute(reader, CVRF_BRANCH);
	xmlTextReaderNextElement(reader);
...
struct cvrf_threat *cvrf_threat_parse(xmlTextReaderPtr reader) {
	...
	threat->threat_type = cvrf_item_parse_type_attribute(reader, CVRF_THREAT);
...
struct cvrf_remediation *cvrf_remediation_parse(xmlTextReaderPtr reader) {
	...
	remed->remed_type = cvrf_item_parse_type_attribute(reader, CVRF_REMEDIATION);
...
struct cvrf_product_status *cvrf_product_status_parse(xmlTextReaderPtr reader) {
	...
	stat->status_type = cvrf_item_parse_type_attribute(reader, CVRF_PRODUCT_STATUS);
	xmlTextReaderNextElement(reader);

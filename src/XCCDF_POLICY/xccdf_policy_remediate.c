...
static int _write_script_header_to_fd(struct xccdf_policy *policy, struct xccdf_result *result, const char *sys, int output_fd)
{
	...
	if (oscap_streq(sys, "urn:xccdf:fix:script:ansible")) {
		...
		if (result == NULL) {
			...
			const char *profile_title = oscap_textlist_get_preferred_plaintext(title_iterator, NULL);
			...
			const char *profile_description = description_iterator != NULL ?
			...
			char *fix_header = oscap_sprintf(
				...
				"###############################################################################\n\n\n",
					...
					xccdf_version_name, oscap_version, profile_id);
			...
			char *fix_header = oscap_sprintf(
				...
				"# 	$ oscap xccdf generate fix --result-id %s xccdf-results.xml \n"
				"# 	$ oscap xccdf generate fix --result-id %s arf.xml \n#\n"
				...
				"###############################################################################\n\n\n",
					...
					end_time, oscap_version, result_id, result_id);

...
static int _write_script_header_to_fd(struct xccdf_policy *policy, struct xccdf_result *result, const char *sys, const char *input_file_name, int output_fd)
{
	if (!(oscap_streq(sys, "") || oscap_streq(sys, "urn:xccdf:fix:script:sh") || oscap_streq(sys, "urn:xccdf:fix:commands") ||
	...
	if (result == NULL) {
		...
		fix_header = oscap_sprintf(
			...
			oscap_version, profile_id, format, input_file_name, remediation_type,
...
int xccdf_policy_generate_fix(struct xccdf_policy *policy, struct xccdf_result *result, const char *sys, const char *input_file_name, int output_fd)
{
	...
	if (result == NULL) {
		...
		if (_write_script_header_to_fd(policy, result, sys, input_file_name, output_fd) != 0) {
			oscap_list_free(rules_to_fix, NULL);
			return 1;
		}

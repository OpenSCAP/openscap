...
static int _write_script_header_to_fd(struct xccdf_policy *policy, struct xccdf_result *result, const char *sys, int output_fd, struct xccdf_profile *profile)
{
	...
	} else if (oscap_streq(sys, "") || oscap_streq(sys, "urn:xccdf:fix:script:sh") || oscap_streq(sys, "urn:xccdf:fix:commands")) {
		// Bash Script
		if (result == NULL) {
			...
			const char *fix_header = oscap_sprintf(
				...
				"# 	$ oscap xccdf generate fix --profile %s sds.xml \n#\n"
				"# This script is generated from an OpenSCAP profile without preliminary evaluation.\n"
				"# It attempts to fix every selected rule, even if the system is already compliant.\n#\n"
				"###############################################################################\n\n\n",
					profile_id, profile_title, profile_description, benchmark_id,
					benchmark_version_info, oscap_version, profile_id);

			return _write_text_to_fd_and_free(output_fd, fix_header);
		} else {
			// Results-based remediation fix
			printf("result is not null- header script function");
			...
			const char *fix_header = oscap_sprintf(
				...
				"# Evaluation Result End Time:  %s\n#\n"
				"# This file was generated by OpenSCAP %s using one of the following:\n"
				"# 	$ oscap xccdf generate fix xccdf-results.xml \n"
...
int xccdf_policy_generate_fix(struct xccdf_policy *policy, struct xccdf_result *result, const char *sys, int output_fd)
{
	...
	if (result == NULL) {
		...
		struct xccdf_benchmark *benchmark = xccdf_policy_get_benchmark(policy);
		struct xccdf_policy *profile = xccdf_policy_get_profile(policy);
		...
		printf("result is not null- generate fix function");
